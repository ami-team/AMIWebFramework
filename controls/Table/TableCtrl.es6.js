...
import twigExport from './assets/twig/export.twig';
	...
	onReady: function()
	{
		...
		amiWebApp.loadResources([
			...
			amiWebApp.appendHTML('body', twigRefineModal).done(() => {
				...
				this.fragmentTableCtrl = twigTableCtrl;
				this.fragmentTable = twigTable;
...
//			});
		...
		amiCommand.execute(`${this.ctx.command2}${this.ctx.enableCache ? ' -cached' : ''}${this.ctx.enableCount ? ' -count' : ''}${this.ctx.hideBigContent ? ' -hideBigContent' : ''}`).done((data) => {
			...
			this.replaceHTML(this.patchId('#FEF9E8D8_D4AB_B545_B394_C12DD5817D61'), this.fragmentTable, {dict: dict}).done(() => {
				...
				parent.find('a[data-orderway="DESC"]').click((e) => {
					...
					const field = e.currentTarget.getAttribute('data-row-name');
	...
	showGroupTab: function(catalog, entity, field)
	{
		...
		amiWebApp.createControlInContainer(this.getParent(), this, 'table', [command], {orderBy: columnName, orderWay: 'ASC', showDetails: this.ctx.showDetails}, this.ctx, 'slack', this.ctx.entity);
