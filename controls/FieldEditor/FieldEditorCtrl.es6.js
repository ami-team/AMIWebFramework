...
$AMIClass('FieldEditorCtrl', {
	...
	onReady: function()
	{
		...
		amiWebApp.appendHTML('body', twigFieldEditorCtrl).done(() => {

			const el1 = $('#A8572167_6898_AD6F_8EAD_9D4E2AEB3550');
			const el2 = $('#F44687A3_036C_9C77_3284_DD495D9F4D7D');

			el1.on('shown.bs.modal', () => {

				el1.find(':input:not([readonly])').first().focus();
			});

			el2.on('shown.bs.modal', () => {

				el2.find(':input:not([readonly])').first().focus();
			});

			this.fragmentFieldList = twigFieldList;
	...
	getInfo: function(primaryCatalog, primaryEntity, primaryField)
	{
		...
		amiCommand.execute(`GetEntityInfo -catalog="${amiWebApp.textToString(primaryCatalog)}" -entity="${amiWebApp.textToString(primaryEntity)}"`).done((data) => {
			...
			for(let i in rows)
			{
				...
				const automatic = amiWebApp.jspath('..field{.@name==="automatic"}.$', rows[i])[0] || 'false';
