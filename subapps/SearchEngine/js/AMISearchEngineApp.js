...
function AMISearchEngineApp() {
	...
	this.onReady = function(userdata) {
		...
		$('#ami_breadcrumb_content').html('<li><a href="">Tools</a></li><li><a href="">Search Engine</a></li>');
	...
	this.refresh = function() {
		...
		amiCommand.execute('SearchQuery -catalog="' + this.catalog + '" -sql="SELECT COUNT(*) AS `nb` FROM `' + this.entity + '` WHERE ' + this.buildFilter() + '"', {context: this}).done(function(data) {
			...
			$('#ami_search_engine_message').html('<div style="background-color: #F5F5F5; border-radius: 8px; padding: 4px;"><button class="btn btn-primary btn-sm" onclick="alert(\'' + amiWebApp.textToHtml(amiWebApp.textToString(command)) + '\');">View selection</button> Number of selected items:' + nb + '</div>');
		});
