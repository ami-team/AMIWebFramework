...
import twigSearchModelerApp from './assets/twig/SearchModelerApp.twig';
import twigInterface        from './assets/twig/interface.twig'       ;
import twigInput            from './assets/twig/input.twig'           ;
...
$AMIClass('SearchModelerApp', {
	...
	onReady: function()
	{
		...
		amiWebApp.replaceHTML('#ami_main_content', twigSearchModelerApp).done(() => {
			...
			const f1 = () => {
				...
				const more = this._parseJson($('#A3D83B42_4FBF_5DAE_6A38_12F1F53493B5').val());

				this.formToJson1(more);

				$('#A3D83B42_4FBF_5DAE_6A38_12F1F53493B5').val(this._dumpJson(more));
			};
			...
			const f2 = () => {
				...
				const more = this._parseJson($('#A78C0694_128B_1AD8_2596_C321DAA4690B').val());

				this.formToJson2(more);

				$('#A78C0694_128B_1AD8_2596_C321DAA4690B').val(this._dumpJson(more));
			};
			...
			$('#F9949786_303E_7A76_D205_4DCC1122C0F0').keyup (f2);
			$('#AF93AE9B_1FE0_585B_AA28_2EBD3E078902').keyup (f2);
	...
	getInterfaceList: function()
	{
		...
		amiCommand.execute('SearchQuery -catalog="self" -entity="router_search_interface" -sql="SELECT `id`, `group`, `name`, `rank`, `json`, `archived` FROM `router_search_interface` ORDER BY `rank` ASC, `group` ASC, `name` ASC"').done((data) => {
			...
			amiWebApp.replaceHTML('#CFB6CA12_2D42_3111_3183_EC1006F7E039', twigInterface, {dict: dict}).done(() => {
	...
	swap: function(ranks1, ranks2)
	{
		for(const id in this.searchInterfaces)
		{
			if(ranks1[id] !== ranks2[id])
			{
				...
				const command = 'UpdateElements -catalog="self" -entity="router_search_interface" -fields="rank" -values=? -keyFields="id" -keyValues=?';

				/*----------------------------------------------------------------------------------------------------*/

				amiWebApp.lock();

				amiCommand.execute(command, {params: [ranks2[id], id]}).done(() => {
	...
	select: function(id)
	{
		...
		amiWebApp.replaceHTML('#DD89D783_6F39_7B3B_3F3F_D875737A5E68', twigInput, {dict: dict}).done(() => {
	...
	editOptions1: function()
	{
		...
		$('#A3D83B42_4FBF_5DAE_6A38_12F1F53493B5').val($('#A3D83B42_4FBF_5DAE_6A38_12F1F53493B5').val());
	...
	jsonToForm2: function(more)
	{
		...
		$('#F9949786_303E_7A76_D205_4DCC1122C0F0').val(more.off !== null ? more.off_label : '@NULL');
		$('#AF93AE9B_1FE0_585B_AA28_2EBD3E078902').val(more.on  !== null ? more.on_label  : '@NULL');
	...
	formToJson2: function(more)
	{
		...
		const off_label = $('#F9949786_303E_7A76_D205_4DCC1122C0F0').val();
		if(off_label && off_label.toUpperCase() !== '@NULL') {
			more.off_label = off_label;
		} else {
			delete more.off_label;
		}

		const on_label = $('#AF93AE9B_1FE0_585B_AA28_2EBD3E078902').val();
		if(on_label && on_label.toUpperCase() !== '@NULL') {
			more.on_label = on_label;
		} else {
			delete more.on_label;
		}
	...
	editOptions2: function(inputCnt, inputType)
	{
		...
		if(inputType === 4) {
			...
			$('#F9949786_303E_7A76_D205_4DCC1122C0F0').prop('disabled', false);
			$('#AF93AE9B_1FE0_585B_AA28_2EBD3E078902').prop('disabled', false);
		}
		else {
			...
			$('#F9949786_303E_7A76_D205_4DCC1122C0F0').prop('disabled', true);
			$('#AF93AE9B_1FE0_585B_AA28_2EBD3E078902').prop('disabled', true);
		}
		...
		$('#A78C0694_128B_1AD8_2596_C321DAA4690B').val($('#C4AAADBC_C3B5_6DDC_851B_F06430CB4F6E_' + inputCnt).val());
	...
	{
		$('#C4AAADBC_C3B5_6DDC_851B_F06430CB4F6E_' + inputCnt).val($('#A78C0694_128B_1AD8_2596_C321DAA4690B').val());
	...
	save: function(mode) // 0: STD, 1: CLONE, 2: SHOW
	{
		...
		const more = $('#A3D83B42_4FBF_5DAE_6A38_12F1F53493B5').val();
		...
		{
			amiCommand.execute('AddUpdateElement -catalog="self" -entity="router_search_interface" -separator="£" -fields="group£name£rank£json£archived" -values=? -keyFields="group£name" -keyValues=?', {params: [`${group}£${name}£${rank}£${JSON.stringify(json)}£${archived}`, `${group}£${name}`]}).done((data, message) => {
...
window.searchModelerApp = new SearchModelerApp();
