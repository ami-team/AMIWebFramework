...
$AMINamespace('amiLogin', /** @lends amiLogin */ {
	...
	_start: function(ssoAutoAuthentication, ssoAuthenticationAllowed, passwordAuthenticationAllowed, certificateAuthenticationAllowed, logoutAllowed, createAccountAllowed, changeInfoAllowed, changePasswordAllowed, changeCertificateAllowed, captchaAllowed, bookmarksAllowed)
	{
		...
		amiWebApp.loadTWIGs([
			amiWebApp.originURL + '/template/AMI/Fragment/login_button.template',
			amiWebApp.originURL + '/template/AMI/Fragment/logout_button.template',
			amiWebApp.originURL + '/template/AMI/Modal/login.template',
		]).done((data) => {
			...
			amiWebApp.appendHTML('body', data[2], {dict: this.flags}).done(() => {
			...
			window.addEventListener('message', (e) => {
				...
				if(this.ssoInfo.signInURL.startsWith(e.origin))
				{
			...
			const __start = (promise) => {

				promise.fail((data, message, userInfo, roleInfo, bookmarkInfo, udpInfo, ssoInfo) => {
			...
			const promise = amiCommand.certLogin().done((data, message, userInfo, roleInfo, bookmarkInfo, udpInfo, ssoInfo) => { 

				if(ssoAutoAuthentication && ssoInfo.signInURL)
				{
					amiWebApp.loadJSONs(ssoInfo.signInURL + '?mode=json').done((data) => {

						__start(amiCommand.passLogin(
							data[0].login,
							data[0].data
						));
	...
	/**
	  * Gets the client `not before` date
	  * @returns {String} The client `not before` date
	  */

	getNotBeforeDate: function()
	{
	...
	/**
	  * Gets the client `not after` date
	  * @returns {String} The client `not after` date
	  */

	getNotAfterDate: function()
	{
	...
	{
		return (this.userInfo.AMIUser || 'guest') !== (this.userInfo.guestUser || 'guest');
	},
	...
	sso: function()
	{
		...
		window.open(this.ssoInfo.signInURL + '?mode=html', 'Single Sign-On', 'menubar=no, status=no, scrollbars=no, width=800, height=450');
	},
	...
	form_attachCert: function()
	{
		...
		return amiCommand.attachCert(user, pass).then((data, message) => {
	...
	form_detachCert: function()
	{
		...
		return amiCommand.detachCert(user, pass).then((data, message) => {
	...
	form_remindPass: function(e)
	{
		...
		amiCommand.resetPass(values['user'], values['captcha_hash'], values['captcha_text']).then((data, message) => {
	...
	form_changePass: function(e)
	{
		...
		amiCommand.changePass(this.userInfo.AMIUser || 'guest', values['old_pass'], values['new_pass']).then((data, message) => {
