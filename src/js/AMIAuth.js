...
class AMIAuth
{
	...
	#update(userInfo, roleInfo, bookmarkInfo, dashboardInfo, awfInfo)
	{
		...
		$('#C3E94F6D_48E0_86C0_3534_691728E492F4').attr('src', awfInfo.privacyPolicyURL || `${amiWebApp.originURL}/docs/privacy_policy.html`);
		$('#E50FF8BD_B0F5_CD72_F9DC_FC2BFA5DBA27').attr('src', awfInfo.privacyPolicyURL || `${amiWebApp.originURL}/docs/privacy_policy.html`);
...
<a class="nav-link text-danger" href="javascript:amiAuth.accountStatus();">
	...
	sso()
	{
		...
		window.open(`${this.#awfInfo.ssoAuthURL || ''}?response_type=code&client_id=${encodeURIComponent(this.#awfInfo.ssoClientId || '')}&redirect_uri=${encodeURIComponent(amiRouter.getOriginURL())}/docs/sso.html`, this.#awfInfo.ssoLabel || 'Single Sign-On', 'menubar=no, status=no, scrollbars=no, width=800, height=450');
	}
